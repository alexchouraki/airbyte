...
class WelkinHealthStream(BaseWelkinHealthStream, ABC):
    ...
    `class WelkinHealthStream(HttpStream, ABC)` which is the current class
    `class Customers(WelkinHealthStream)` contains behavior to pull data for customers using v1/customers
    `class Employees(WelkinHealthStream)` contains behavior to pull data for employees using v1/employees

    If some streams implement incremental sync, it is typical to create another class
    `class IncrementalWelkinHealthStream((WelkinHealthStream), ABC)` then have concrete stream implementations extend it. An example
    is provided below.
